function [u] = AdvectionDG1D(x,u,h,m,N,CFL,FinalTime)
% function [u] = AdvectionDG1D(x,u,h,m,N,CFL,FinalTime)
...
while (time<FinalTime)
  ...
  % Update solution
  rhsu  = AdvectionDGrhs1D(x,u,h,k,m,N,Ma,S,VtoE,1.0); 
  u1 = u + k*rhsu; 
  u1 = WENOlimitDG(x,u1,m,h,N,V,iV,Q,Xm,Xp);
  
  rhsu  = AdvectionDGrhs1D(x,u1,h,k,m,N,Ma,S,VtoE,1.0);
  u2 = (3*u + u1 + k*rhsu)/4; 
  u2 = WENOlimitDG(x,u2,m,h,N,V,iV,Q,Xm,Xp);

  rhsu  = AdvectionDGrhs1D(x,u2,h,k,m,N,Ma,S,VtoE,1.0); 
  u = (u + 2*u2 + 2*k*rhsu)/3; 
